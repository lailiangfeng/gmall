...
import com.atguigu.gmall.oms.feign.GmalllPmsClient;
...
public class OrderServiceImpl extends ServiceImpl<OrderDao, OrderEntity> implements OrderService {
    ...
    @Autowired
    private GmalllPmsClient gmalllPmsClient;
    @Autowired
    ...
    public OrderEntity saveOrder(OrderSubmitVO submitVO) {
        ...
        Resp<MemberEntity> memberEntityResp = this.gmallUmsClient.queryMemberById(submitVO.getUserId());
        ...
        //保存订单详情表OrderitemEntity
        List<OrderItemVO> items = submitVO.getItems();
        items.forEach(item->{
            OrderItemEntity itemEntity = new OrderItemEntity();
            itemEntity.setSkuId(item.getSkuId());
            Resp<SkuInfoEntity> skuInfoEntityResp = this.gmalllPmsClient.querySkuById(item.getSkuId());
            SkuInfoEntity skuInfoEntity = skuInfoEntityResp.getData();

            Resp<SpuInfoEntity> spuInfoEntityResp = this.gmalllPmsClient.querySpuById(skuInfoEntity.getSpuId());
